...
function PixiePlot:Redraw()
	...
	if self.nDataSets > 0 then
		...
		for _,dataSet in pairs(self.dataSets) do
			...
			self:UpdateMinMaxValuesAlt(dataSet)
...
function PixiePlot:PlotDataSet(dataSet, color, nBarIndex)
	...
	if ePlotStyle == self.LINE or ePlotStyle == self.STEM then
		...
		if self.tOpt.eCoordinateSystem == self.CARTESIAN then
			local xOffset = nil
			if dataSet.xStart ~= nil then
				xOffset = (dataSet.xStart - self.fXMin) / fXRange * maxWidth
			else
				xOffset = (dataSet.xEnd + self.fXMax) / fXRange * maxWidth
			end
			--Print(xOffset)
			local xIntervalWidth = maxWidth / (dataSet.values.last - dataSet.values.first)
			local vPrev
			-- Line
			local i = 0
			local v = nil
			for q = dataSet.values.first, dataSet.values.last do
				v = dataSet.values[q]
				i = i + 1
	...
	elseif ePlotStyle == self.SCATTER then
		...
		for _, v in pairs(dataSet.values) do
...
-- @param size Width and height of the tooltip. Should be the same dimensions as your symbols.
function PixiePlot:AddWindowOverlay(pos, size)
	if not self.tOpt.xmlWndOverlay then return end
	--Print("Drawing tooltip at: " .. pos.x .. ", " .. pos.y)
	local wnd = Apollo.LoadForm(self.tOpt.xmlWndOverlay, "WindowOverlay", self.wnd, self)
...
-- do not update x and y values on loop of a size of the data set every refresh
-- multiple data sets not implemented
function PixiePlot:UpdateMinMaxValuesAlt(dataSet)
	if dataSet.xStart ~= nil then
		local x = self.fXMin + (dataSet.values.last - dataSet.values.first + 1) * self.fXInterval
		if self.fXMax < x then self.fXMax = x end
	else
		local x = self.fXMax - (dataSet.values.last - dataSet.values.first + 1) * self.fXInterval
		if self.fXMin > x then self.fXMin = x end
	end
	
	local fYLast = dataSet.values[dataSet.values.last]
	if fYLast > self.fYMax then self.fYMax = fYLast end
	
	-- this is THE SAME float, so we can compare equality (should be able to compare)
	if dataSet.values.first > 1 and dataSet.fYFirst >= self.fYMax then
		local fYMax = 0
		for q = dataSet.values.first, dataSet.values.last do
			local y = dataSet.values[q]
			if y > fYMax then fYMax = y end
		end
		self.fYMax = fYMax
		--Print("new Y max")
	end
	dataSet.fYFirst = dataSet.values[dataSet.values.first]
end
...
function PixiePlot:UpdateMinMaxValues(dataSet)
	if self.tOpt.ePlotStyle == self.LINE or self.tOpt.ePlotStyle == self.STEM then
		...
		for q = dataSet.values.first, dataSet.values.last do
			...
			local x = nil
			if dataSet.xStart ~= nil then
				x = dataSet.xStart + (i - 1) * self.fXInterval
			else
				x = dataSet.xEnd - (i - 1) * self.fXInterval
			end
			if self.tOpt.eCoordinateSystem == self.POLAR then
	...
	__call = function(self, wndContainer, tOpt)
		...
		if tOpt then
			...
			pp.tOpt = {
				...
				strXLabel = "",
				strYLabel = "",
				bDrawXAxisLabel = true,
				bDrawYAxisLabel = true,
				nXValueLabels = 8,
				nYValueLabels = 8,
				...
				xmlWndOverlay = XmlDoc.CreateFromFile("PixiePlot.xml"),
				bWndOverlays = false,
				fWndOverlaySize = 6,
...
_G.AddonsManagerStuff.PixiePlot = PixiePlot
